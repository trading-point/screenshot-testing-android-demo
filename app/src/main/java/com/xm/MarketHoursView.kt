 ...
 *
 * A view for representing the market sessions on timeline bar.
 * The step between hour indicator is two hours.
 */
class MarketHoursView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : ConstraintLayout(context, attrs, defStyleAttr) {
    ...
    private val mCurrentLinePaint: Paint = Paint()
    private val mTineRangePaint: Paint = Paint()

    private lateinit var mMarketDisplayInfo: MarketDisplayInfo
    ...
    init {
        ...
        mCurrentLinePaint.color = ContextCompat.getColor(context, R.color.lineColor)
        mCurrentLinePaint.isAntiAlias = true
        mCurrentLinePaint.style = Paint.Style.STROKE
        mCurrentLinePaint.strokeWidth = context.resources.getDimensionPixelSize(R.dimen.market_hours_current_time_line_width).toFloat()

        mTineRangePaint.color = ContextCompat.getColor(context, R.color.green)
        mTineRangePaint.isAntiAlias = true
        mTineRangePaint.style = Paint.Style.FILL
        mTineRangePaint.strokeWidth = context.resources.getDimensionPixelSize(R.dimen.market_hours_time_bar_height).toFloat()
    }
    ...
    fun setMarketDisplayInfo(marketDisplayInfo: MarketDisplayInfo) {
        mMarketDisplayInfo = marketDisplayInfo
    ...
    private fun drawTimeRanges(canvas: Canvas) {
        ...
        mMarketDisplayInfo.marketHoursSessions.forEach {
            val lineRange = getDisplayedTimeRange(it)
            canvas.drawLine(lineRange.start, barPosition, lineRange.end, barPosition, mTineRangePaint)
        }
    }
    ...
    private fun drawCurrentTimeLine(canvas: Canvas) {
        ...
        val normalizedCurrentTime = normalizeTime(mMarketDisplayInfo.currentTime.hour,
            mMarketDisplayInfo.currentTime.minute,
            findHourViewsRange(mMarketDisplayInfo.currentTime.hour))
        // calculate the exact x position
        val lineXPosition = when (mMarketDisplayInfo.currentTime.hour) {
            0, 24 -> normalizedCurrentTime
            else -> normalizedCurrentTime + (mCurrentLinePaint.strokeWidth / 2)     //center line to the time line point
        }
        ...
        canvas.drawLine(lineXPosition, lineYStartPosition, lineXPosition, lineYEndPosition, mCurrentLinePaint)
    }
    ...
    /**
     * A method to convert hours an minutes, only in minutes
     */
